...
def p_error(p):
    ...
    parser.resultado_sintactico = "Error de sintaxis"
...
@app.route('/')
def index():
    return render_template('index.html', resultado_lexico='', resultado_sintactico='', arbol='')
...
def analizar():
    ...
    parser.resultado_sintactico = None
    arbol = parser.parse(expresion)
    if parser.resultado_sintactico == "Error de sintaxis":
        return jsonify({
            'resultado_lexico': resultado_lexico,
            'resultado_sintactico': parser.resultado_sintactico,
            'arbol': ''
        })
    else:
        return jsonify({
            'resultado_lexico': resultado_lexico,
            'resultado_sintactico': "Operación analizada correctamente",
            'arbol': RenderTree(arbol).by_attr()
        })
